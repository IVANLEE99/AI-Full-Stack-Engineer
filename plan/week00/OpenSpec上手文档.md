...
`/opsx:verify` 只做一件事：拿实现结果去对照 `specs / tasks / design`，报出不一致、遗漏或偏差。它本身不改代码，输出的是一份「差在哪」的清单。所以 verify 之后要不要优化、怎么优化，取决于它报出来的是哪一类问题。

### 16.1 三种差异，三种处理方式

**A. 实现没做到位（最常见）**

代码和规格对不上，是代码欠债，不是需求变了。

- 回到 `/opsx:apply` 继续改。verify 报的每一条差异，基本对应 `tasks.md` 里没完成或做错的项。
- 改完再跑一次 `/opsx:verify`，形成 `apply → verify → apply` 的小循环，直到 verify 干净。
- 这一步**不要动规格**。

**B. 规格本身写错了 / 想法变了**

verify 让你发现当初的 `specs` 或 `design` 定义得不合理，这是规划的问题。

- 回头改工件，而不是硬改代码去迁就错的规格。改的优先级从上游往下：
  - 目的 / 范围变了 → 改 `proposal.md`
  - 行为定义变了 → 改 `specs/` 里的 delta
  - 技术方案变了 → 改 `design.md`
  - 步骤变了 → 更新 `tasks.md`
- 这些文档是「支撑物」不是硬门槛，实现中发现问题回头改是正常操作。
- 改完规格后再 `/opsx:apply` 让代码跟上，然后再 `/opsx:verify`。

**C. 差异是合理的、可接受的**

有些 verify 报的「偏差」其实是你有意为之（比如某个任务本次不做、留到下个 change）。

- 更新 `tasks.md`，把不做的项说明清楚或拆到新的 change，让规格和现实重新对齐。
- 别留着一堆「已知不一致」直接进 archive，否则归档后主 `specs/` 就成了不可信基线。

### 16.2 判断该改哪边的简单原则

问一句：**「是代码没达到规格，还是规格本身不对？」**

- 代码没达到 → 改代码（`apply`）
- 规格不对 → 改规格（`proposal / specs / design`），再让代码跟上
...
```text
        ┌─────────────────────────────┐
        ▼                             │
/opsx:apply ──► /opsx:verify ──► 有差异？
        ▲                             │
        │        代码没达到 ──────────┘
        │        规格不对 ──► 改 proposal/specs/design ──┘
        │
     verify 通过
        │
        ▼
...
优化到 `/opsx:verify` 不再报关键问题后，再走 `/opsx:archive`。archive 会把这次的 delta 合并回主 `specs/`，所以**必须在 verify 通过后再归档**，否则错误的基线会被固化下来。
