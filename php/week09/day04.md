...
### 8.1 OAuth 第三方登录的本质是什么？

参考答案：

> ✅ 第三方平台（如 Google/Facebook）负责证明“这个外部账号是谁”，你的系统再把它绑定到本地 `user_id`，最终仍然由你自己的用户服务签发本系统的 token。第三方的 token 不是你系统的登录凭证。

---

### 8.2 `client_secret` 为什么必须放后端，不能放前端？

参考答案：

> ✅ `client_secret` 是第三方平台分配的密钥，用来换取 access token。如果放在前端，任何人都能看到并盗用你的应用身份，冒充你去调用第三方接口。所以它只能存在后端配置里。

---

### 8.3 OAuth 流程里 `code` 和 `access_token` 有什么区别？

参考答案：

> ✅ `code` 是授权码，用户同意授权后第三方回调时带回来，只能短时使用一次；后端拿 `code` + `client_secret` 去换 `access_token`；`access_token` 才是真正用来调第三方 API 获取用户资料的凭证。
...
> ✅ `state` 是一个随机值，用来防止 CSRF 攻击。发起授权时生成并保存，回调时校验回传的 `state` 是否一致，确保这次回调是你自己发起的，而不是攻击者伪造的。

---

### 8.5 为什么不建议只用 email 判断第三方账号归属？

参考答案：

> ✅ 因为第三方邮箱可能未验证、可能被用户修改、不同平台还可能出现相同邮箱，只靠 email 匹配容易绑错用户。更可靠的做法是用 `provider` + `provider_user_id` 这个唯一组合来定位绑定关系。
