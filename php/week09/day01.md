...
> ✅ 因为登录成功后生成的 `user_id` / token 会贯穿 BFF、订单归属、支付归属、通知和权限判断。身份一旦错了，就会出现 A 用户看 B 用户订单、越权支付、资料泄露等严重问题，所以一切业务安全都从身份边界开始。

---

### 8.2 Controller 层在登录链路里应该负责什么？

参考答案：

> ✅ Controller 只负责入口层的活：接收参数（手机号、验证码、第三方 code）、做基础校验（参数是否存在、格式是否正确）、调用 Service、返回 `code/data/info`。真正的身份核心规则要放在 Service 或 Node 链里。

---

### 8.3 quickLogin（快捷登录）要解决哪三个关键问题？

参考答案：

> ✅ 一是如何确认这个登录凭证是真的（校验验证码/第三方 code/运营商 token）；二是如何找到或创建对应用户；三是如何返回一个可信的 token/session。

---

### 8.4 登录成功返回的 `code/data/info` 里，你要重点记录哪些字段？

参考答案：

> ✅ 要记录 token 字段叫什么、user_id 字段叫什么、是否返回过期时间、用户资料是否脱敏、错误码如何区分成功和失败。

---

### 8.5 为什么不能只靠前端传的 `user_id` 来查订单或支付？

参考答案：

> ✅ 因为前端传的 `user_id` 可以被伪造，直接信任会导致越权。正确方式是从 token/session 解析出当前用户，由后端自己注入 `user_id`，并在查询时按这个 `user_id` 限制资源归属。
