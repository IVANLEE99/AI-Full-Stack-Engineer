...
### 8.1 什么是“从前端 API 反查后端链路”？

参考答案：

> ✅ 就是从浏览器或前端代码里的 URL 开始，沿着网关路由、Controller、鉴权/Filter、公参注入、HTTP Request 类一路追到内网服务，最终搞清楚这个接口到底经过了哪些层。

---

### 8.2 鉴权解决的是什么问题？常见方式有哪些？

参考答案：

> ✅ 鉴权解决的是“当前请求是谁发的、有没有权限访问”的问题。常见方式有 token、session、JWT、API key。BFF 网关常见做法是读取 token → 校验有效性 → 得到 user_id → 传给后续服务。

---

### 8.3 白名单是什么？是否等于“安全放行”？

参考答案：

> ✅ 白名单是指可以跳过普通登录鉴权的接口，比如登录、注册、首页配置。但它不等于安全放行，像支付回调这类接口即使在白名单里，也必须做签名校验或来源校验。

---

### 8.4 什么是公参注入？它解决了什么麻烦？

参考答案：

> ✅ 公参就是很多接口都需要的公共参数，如 user_id、site_id、lang、trace_id。公参注入是在请求进入后统一把这些信息放进上下文，避免每个 action 重复解析。类似 Node 里用 middleware 统一设置 `req.context`。

---

### 8.5 反查 `pay/pay/methods` 时，链路上大致会经过哪些节点？

参考答案：

> ✅ 前端 `/pay/pay/methods` → 网关鉴权/Filter（做登录校验和公参注入）→ `PayController::actionMethods()` → `PayRequest::methods()` → 内网支付服务 `/pay/methods`。
