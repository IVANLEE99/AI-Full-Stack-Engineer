...
### 8.1 为什么 BFF 网关需要一份 API 路由表？

参考答案：

> ✅ 因为网关里的接口很多，路由表能把前端 URL、网关 Controller/action、HTTP Client、目标服务、鉴权等信息整理到一张表里，方便新人 onboarding 和快速反查代码、排查问题。它不是为了写文档而写文档，而是为了能快速定位链路。

---

### 8.2 一份结账 API 路由表至少应该包含哪些字段？

参考答案：

> ✅ 至少包含：前端 URL、HTTP 方法、网关模块、Controller/action、是否鉴权、公参、HTTP Client（如 PayRequest）、目标服务、内网 path、备注。这样才能从前端一路反查到内网服务。

---

### 8.3 填路由记录时遇到暂时确认不了的字段该怎么办？

参考答案：

> ✅ 写“待确认”，绝不能编造。路由表的价值在于准确反查代码，写错反而会误导排查。

---

### 8.4 结账链路的接口相比普通展示接口，需要额外标注哪些风险？

参考答案：

> ✅ 需要标注鉴权（未登录不能下单/支付）、幂等（重复点击不能重复下单）、金额（必须由后端计算，不信前端）、状态（以后端/支付回调为准）、日志（失败要能追踪）。因为结账涉及订单和资金，风险更高。

---

### 8.5 拿到一条路由记录（如 `/order/order/create`）后，如何用它反查代码？

参考答案：

> ✅ 先找 `OrderController::actionCreate()`，看它取了哪些参数、是否 `requireLogin()`，再看它调用哪个 `OrderRequest` 方法，跳进去看内网 path 和目标服务。路由表写得好，1-2 分钟内就能定位到关键代码。
