...
**延伸阅读**：[day03-源码阅读笔记](./day03-源码阅读笔记.md)（基于 `PayController` 的 action 行数统计、五类拆分与薄度判断）
...
### 8.1 用一句话说清楚什么是“薄 Controller”？

参考答案：薄 Controller 指网关 action 里只保留入口层必须做的事——鉴权、取参、基础校验、调用服务、返回响应，不承载核心业务规则。

### 8.2 网关 Controller 适合做哪 5 件事？

参考答案：鉴权（判断是否登录）、取参（读取请求参数）、基础校验（必填/类型/格式）、调用 Request/Service 完成业务、用统一格式返回响应。

### 8.3 为什么网关 Controller 要保持薄？

参考答案：BFF 网关不是核心业务服务。Controller 写太厚会导致业务规则在网关和服务里重复、修改时不知道改哪层、难测试、难多端复用，网关容易变成“大泥球”。

### 8.4 举出至少 3 个不应该出现在网关 Controller 的逻辑，并说明应放在哪里。

参考答案：订单金额计算（订单/价格服务）、库存扣减（库存/商品服务）、支付状态流转（支付服务）、优惠券核销（营销服务）、数据库事务（具体业务服务），这些都是核心业务规则，需要事务、一致性或复用。

### 8.5 判断一个 action 是否偏厚，有什么简单信号？

参考答案：把每段代码分类，若出现大量复杂业务判断、直接 SQL 查询或金额/状态计算，且行数很长（比如超过 50-80 行），就要警惕它已经不薄了。
