...
### 8.1 `OrderService` 在订单域里扮演什么角色？

参考答案：

> ✅ 它是订单业务的编排中心。Controller 只负责入口，Repository/Model 只负责数据，而订单创建、确认、状态判断、返回结构组织通常都由 Service 承接。

---

### 8.2 `['code','data','info']` 三个字段分别是什么含义？

参考答案：

> ✅ `code` 是业务状态码（0 通常表示成功），`data` 是成功时返回的数据，`info` 是成功或失败说明。注意它是业务响应包装，不等同于 HTTP 状态码。

---

### 8.3 Service 和 Repository 的分工是什么？

参考答案：

> ✅ Service 关心业务判断和编排（是否可下单、组织返回结构、调用多个数据源），Repository 关心具体的数据查询和写入。

---

### 8.4 哪些逻辑不适合放在 Service 里？

参考答案：

> ✅ 直接处理 HTTP request/response、写前端展示逻辑、拼接路由、负责 Controller 鉴权入口，这些都不该放在 Service。

---

### 8.5 读 `OrderService` 源码时，先做什么最有效？

参考答案：

> ✅ 先列出所有 public 方法，记录方法名、参数、返回值，建立“方法地图”，不要一上来就钻进每个私有方法的细节。
