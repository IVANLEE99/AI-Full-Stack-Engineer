...
### 8.1 什么是错误传递？为什么不能把下游错误原样返回给用户？

参考答案：

> ✅ 错误传递是把下游错误“翻译”成上游能理解、用户能接受的错误，而不是原样透传。比如支付渠道返回 `curl error 28: operation timed out`，直接给用户看毫无意义还暴露内部细节，应转换成“支付处理中，请稍后查看订单状态”，同时在日志里保留原始错误码用于排障。

---

### 8.2 跨服务链路里的四类错误分别是什么？哪些能直接给用户看？

参考答案：

> ✅ 四类：用户可见错误（库存不足、订单已取消，可以直接展示）、业务错误（订单状态不允许支付，转换后展示）、系统错误（数据库连接失败、代码异常，只提示“系统繁忙”）、下游原始错误（curl 错误、渠道错误码，不直接展示）。只有前两类适合转换后给用户。

---

### 8.3 `trace_id`、`request_id`、业务 ID 有什么区别？

参考答案：

> ✅ `trace_id` 是全链路追踪 ID，一次请求经过所有服务时保持同一个，用来把多服务日志串起来；`request_id` 是某个服务或某次 HTTP 调用自己的 ID，每一跳可能不同；业务 ID 是订单号、支付单号、售后单号这类领域内的标识。三者配合才能从整条链路定位到具体失败点。

---

### 8.4 统一错误结构里为什么要保留 `downstream` 字段？

参考答案：

> ✅ 因为它记录了下游服务名、下游 request_id 和下游原始错误码。这样即使当前服务只给用户返回了友好 message，排障时也能顺着 downstream 信息跳到下游服务的日志，快速定位是哪个下游、哪次请求出的问题。

---

### 8.5 `trace_id` 应该在哪些地方传递才能保证全链路可追踪？

参考答案：

> ✅ 要在同步 HTTP 请求头（如 `X-Trace-Id`）、每个服务的日志、异步的 MQ 消息和 Webhook 回调里都带上同一个 trace_id。任何一跳丢失 trace_id，链路就会断开，排障时那一段就变成盲区。
