...
### 8.1 完整结账链路只有“下单接口 + 支付接口”吗？

参考答案：

> ✅ 不是。完整链路是从前端结账、网关转发、订单创建、支付发起（同步阶段），再到 Webhook 回调、MQ 投递、消费者处理、订单/通知/权益更新（异步阶段）的一整条同步 + 异步链路。

---

### 8.2 结账链路里哪些步骤是同步，哪些是异步？

参考答案：

> ✅ 同步阶段：前端 → BFF 网关 → 订单服务 confirm/place → 支付服务发起支付（返回 client_secret / redirect_url）。异步阶段：支付平台 Webhook 回调 → 验签/幂等/金额校验 → 投递 MQ 事件 → 订单/通知/权益消费者处理。

---

### 8.3 “发起支付成功”等于“支付真正成功”吗？

参考答案：

> ✅ 不等于。发起支付成功只是创建了 PaymentIntent/交易并返回 client_secret 或跳转地址；支付是否真正成功要等第三方支付平台的 Webhook 回调确认。

---

### 8.4 Webhook 回调里为什么要快速返回 2xx？

参考答案：

> ✅ 因为支付平台对回调有超时限制，处理太慢会被判定失败并触发重试。所以 Webhook 里只做验签、幂等、金额校验和更新支付单、投递 MQ 这些轻动作，把更新订单、发通知、发权益等耗时动作交给消费者异步处理。
...
> ✅ 金额后端计算（不信前端）、库存校验、下单锁/幂等、支付回调验签、回调金额校验、Webhook 快速返回、MQ 投递失败重试、消费者幂等、死信队列/失败补偿等。至少能列出 8 个。
