...
### 8.1 为什么支付成功后不该把所有后续动作都放在 Webhook 里同步完成？

参考答案：

> ✅ 因为后续动作很多（发通知、发权益、通知其他系统等），全部同步执行会让 Webhook 响应变慢，支付平台可能超时并重试；任何一步失败都可能拖垮整个回调。用 MQ 把事件投递出去后快速返回，让消费者异步慢慢处理，实现异步解耦。

---

### 8.2 RabbitMQ 里 producer、exchange、route_key、queue、consumer 分别是什么角色？

参考答案：

> ✅ producer 是生产者（发消息），exchange 是交换机（决定消息怎么路由），route_key 是路由键（说明消息类型，如 `payment.succeeded`），queue 是队列（保存消息的收件箱），consumer 是消费者（处理队列里的消息）。

---

### 8.3 exchange 和 route_key 一起决定了什么？

参考答案：

> ✅ 它们一起决定消息被投递到哪些队列。RabbitMQ 根据 exchange 加上 route_key 的绑定规则，把一条消息路由到一个或多个队列，从而让一个事件被多个消费者处理，这就是事件驱动的基础。

---

### 8.4 支付成功事件消息里为什么要带 `event_id`？

参考答案：

> ✅ `event_id` 是事件的唯一 ID，用于幂等。消费者可以用它判断这条消息是不是已经处理过，从而避免重复消息导致重复更新订单、重复发权益。

---

### 8.5 用了 MQ 之后，消费者为什么还必须能处理重复消息？

参考答案：

> ✅ 因为网络重试、broker 重投等原因，MQ 可能把同一条消息投递多次。用了 MQ 不代表万事大吉，消费者必须做到幂等，保证重复消息不会造成重复的业务结果。
