...
### 8.1 Yii2 和 TP8 虽然写法不同，但核心工程问题相同，是指什么？

参考答案：

> ✅ 请求入口、参数校验、业务编排、数据查询、鉴权中间件和服务调用都需要清晰分层。你要迁移的是“读代码方法”，而不是死记框架 API。

---

### 8.2 Yii2 的 scenarios 对应 TP8 的什么？

参考答案：

> ✅ 对应 TP8 Validate 的 `$scene`（或 scene 方法），都是让同一个资源在不同接口中校验不同字段组合。

---

### 8.3 InternalServiceHelper 一般负责什么？

参考答案：

> ✅ 封装内部服务调用，通常负责拼接内部服务 URL、添加内部鉴权 header、统一超时、统一解析响应、记录日志和处理错误。它类似 Week 05 学过的 HTTP Client 封装。

---

### 8.4 进入一个新的 PHP 框架项目，应该按什么顺序读代码？

参考答案：

> ✅ 先找 README，再找入口和路由，然后依次找 Controller、参数校验、Service、Model/ORM、中间件/鉴权、内部服务调用封装，最后看统一响应和错误处理。

---

### 8.5 用 NestJS 类比，Yii2 的 AR/Repository 和 TP8 的 Model/ModelJoin 对应什么？

参考答案：

> ✅ 都对应数据层，NestJS 里是 Repository/Prisma。三者都负责查询和持久化，只是实现方式不同。
