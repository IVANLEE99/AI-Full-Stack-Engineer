...
### 8.1 配置 API 的全链路一般包含哪几步？

参考答案：前端请求配置接口 → Controller 接收请求 → Service 组织业务配置 → Helper/`g_config()` 读取配置 → Controller 返回统一 JSON 给前端。

### 8.2 在配置 API 链路里，Controller 主要负责什么？

参考答案：接收请求、做基础参数校验、调用 Service，并把结果包装成统一响应返回，不写大量业务拼装逻辑。

### 8.3 `ConfigService` 在链路里负责什么？

参考答案：组织配置字段、通过 `g_config()` 读取业务配置，并决定返回给前端的数据结构。

### 8.4 前端字段 `site_name` 和后端配置 `site.name` 一定同名吗？

参考答案：不一定。前端看到的字段名（如 `site_name`）可能和后端配置 key（如 `site.name`）不同，由 Service 负责映射。

### 8.5 统一 JSON 响应里的 `code`、`message`、`data` 分别是什么？

参考答案：`code` 表示请求是否成功，`message` 是提示信息，`data` 是真正的业务数据。
