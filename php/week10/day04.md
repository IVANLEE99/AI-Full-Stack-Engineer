...
### 8.1 API 和 Console 的核心区别是什么？

参考答案：

> ✅ API 面向用户或系统的实时 HTTP 请求，要快速返回结果；Console 面向定时任务、批处理、补偿和后台维护，运行在命令行环境，可以长时间执行。

---

### 8.2 Yii Console 命令的一般格式是什么？
...
> ✅ 例如 `php yii oms/handle-timeout` 会调用 OmsController 的 actionHandleTimeout 方法。

---

### 8.3 售后域中哪些场景更适合用 Console？

参考答案：

> ✅ 超时未退货自动关闭、退款状态批量同步、MQ 失败补偿重试、售后数据修复、批量通知、与外部 OMS 定时对账等，这些都是定时批量、耗时或不需要用户实时响应的任务。

---

### 8.4 Console 批处理任务最需要注意哪些风险？

参考答案：

> ✅ 重复执行（需幂等设计和锁）、批量误操作（需 dry-run、limit、日志）、执行时间过长（需分页分批）、缺少鉴权保护、日志不足难排查。

---

### 8.5 Console 和 MQ 如何配合？

参考答案：

> ✅ Console 可以定时扫描 MQ 消费失败的任务并重试，也可以定时扫描超时售后单后投递事件给消费者处理关闭和通知，二者都属于后台异步处理入口。
