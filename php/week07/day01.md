...
### 8.1 支付域为什么要把接口分成 client、internal、outer 三类？

参考答案：

> ✅ 因为调用方不同，安全模型也不同。client 面向用户端，需要用户登录态；internal 面向内网服务，需要服务间鉴权；outer 面向第三方回调，必须验签。分类是为了对不同调用方用不同的信任与校验方式。

---

### 8.2 client 类接口的典型鉴权方式是什么？

参考答案：

> ✅ 通常需要用户登录态或网关鉴权，也就是要知道“是谁在支付”，从而防止越权支付他人订单。

---

### 8.3 outer 回调接口为什么不能只靠 URL 保密或内网白名单？

参考答案：

> ✅ 因为第三方支付平台没有用户 session，且回调 URL 可能被泄露或猜到，只有验证平台签名才能确认回调确实来自支付平台，防止被伪造。

---

### 8.4 outer 回调为什么必须做幂等？

参考答案：

> ✅ 因为第三方平台可能对同一笔支付重复通知。如果不做幂等，同一事件会被处理多次，导致重复更新订单、重复发货或重复发放权益。
...
> ✅ 一个订单可能对应一个或多个支付尝试（换支付方式或失败重试）。支付接口通常要带 order_id、pay_id、amount、currency、channel、transaction_id 等字段来关联两者。
