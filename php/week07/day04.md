...
### 8.1 支付流程为什么要前后端分工，而不是全放前端？

参考答案：

> ✅ 支付涉及敏感信息和资金。前端适合做交互（展示、收集卡信息、跳转），后端必须做可信业务判断（校验订单和金额、调用支付平台、验签回调、更新状态）。前端不能决定金额或支付结果。

---

### 8.2 Stripe 的 PaymentIntent 可以怎么理解？金额由谁决定？

参考答案：

> ✅ PaymentIntent 是一次支付意图/一次准备支付的记录，由后端创建。金额必须由后端根据订单重新计算，绝不能相信前端传来的金额。

---

### 8.3 后端 SDK Service（如 StripeService）通常封装哪些能力？

参考答案：

> ✅ 常见有：创建支付意图、确认支付、查询支付状态、发起退款、验证回调签名、解析回调事件。目标是把第三方 SDK/API 包装成项目内部可理解的方法。
...
> ✅ 支付平台 secret key、webhook signing secret、商户私钥、服务间 token、内部改支付单状态的接口。判断原则是：能直接操作商户资金或伪造支付结果的东西绝不能放前端。

---

### 8.5 为什么支付结果要以后端收到的回调为准，而不是前端提示？

参考答案：

> ✅ 前端说“支付成功”不一定可信，可能被伪造或中途中断。最终可信来源是支付平台回调，后端要验签、解析事件、校验金额币种、幂等处理后，才更新支付单和订单状态。
