...
### 8.1 支付系统为什么需要 PaymentFactory 这样的渠道工厂？

参考答案：

> ✅ 因为支付通常有 Stripe、Braintree、PayPal 等多个渠道。如果到处写 if/else 判断渠道，新增渠道要改很多地方且难维护。工厂把“选哪个类”集中起来，业务代码只说要用哪个渠道即可。
...
> ✅ 我告诉工厂我要什么类型，工厂返回对应的对象。调用方不关心具体类名、怎么 new、依赖什么 SDK。

---

### 8.3 支付方式、支付公司、SDK Service 有什么区别？

参考答案：

> ✅ 支付方式是用户看到的入口（如信用卡、Apple Pay）；支付公司/渠道是背后的服务商（如 Stripe、Braintree）；SDK Service 是项目里封装的调用类（如 StripeService）。

---

### 8.4 新增一个支付渠道只需要新建一个 Service 类吗？

参考答案：

> ✅ 不是。除了新建 Service 类，通常还要加渠道常量、配置项（key、回调密钥）、Factory 映射、回调 Controller、状态映射，并补充成功/失败/重复回调/金额不一致等测试。

---

### 8.5 PaymentFactory 可以类比 Node.js 里的什么？

参考答案：

> ✅ 可以类比 handlers map（策略注册表）。用渠道名作为 key 从映射里取出对应 handler，取不到就抛“不支持的渠道”错误，思路和 PHP 的 PaymentFactory 一致。
