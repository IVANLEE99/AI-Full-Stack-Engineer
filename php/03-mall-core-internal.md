# 商城核心服务 `internal.bm.com` 新手上手指南

> 面向读者：刚接触 PHP、Yii2 和公司商城后端的开发者  
> 仓库路径：`youngs/internal.bm.com`  
...
结论：不要在新手文档中承诺“一条 Docker 命令即可启动”。第一次启动前必须确认团队维护的环境仓库或 Compose 文件位置。
...
新手常见误区是把 Model 当成业务 Service。这个项目明确要求 Controller/Service 通过 Repository 操作 Model。
...
```bash
curl -X POST 'http://<本地模块域名>/<controller-route>/<action-route>' \
